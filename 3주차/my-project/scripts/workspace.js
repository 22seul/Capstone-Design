...
onAuthStateChanged(auth, (user) => {
    if (user) {
        // 닉네임이 있다면 표시, 없으면 기본값 설정
        const nickname = user.displayName || "사용자";
        document.getElementById("nickname-placeholder").textContent = nickname;
    } else {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelector(".workspace-header").textContent = `${nickname}'s WORKSPACE`;
});
